    ...
    public class FargoSoulsGlobalNPC : GlobalNPC
    {
        ...
        public override void SetDefaults(NPC npc)
        {
            ...
            if (FargoSoulsWorld.MasochistMode)
            {
                ...
                /*if (npc.type == ModContent.oader.GetMod("Fargowiltas").NPCType<Destroyer>())
                {
                ...
                switch (npc.type)
                {
                                    ...
                                    if (++Timer > 100)
                                    {
                                        ...
                                        if (Main.netMode != 1)
                                        {
                                            ...
                                            for (int i = 0; i < max; i++)
                                            {
                                                ...
                                                Projectile.NewProjectile(spawnPos, speed, ModContent.ProjectileType<DestroyerLaser>(), npc.damage / 5, 0f, Main.myPlayer);
                                            }
                                        }
                                    }
                    ...
                    /*case NPCID.Probe:
                        if (npc.HasValidTarget && ++Counter > 120)
                        {
                            ...
                            distance.Normalize();
                            distance *= 8f;
                            Projectile.NewProjectile(npc.Center, distance, ModContent.ProjectileType<DarkStar>(), npc.damage / 4, 0f, Main.myPlayer);
                        }
                        break;*/
        ...
        public override void OnHitPlayer(NPC npc, Player target, int damage, bool crit)
        {
            if (FargoSoulsWorld.MasochistMode)
            {
                switch (npc.type)
                {
                    ...
                    case NPCID.IlluminantBat:
                        target.AddBuff(ModContent.BuffType<MutantNibble>(), 1800);
                        target.AddBuff(BuffID.Rabies, 3600);
                        break;
