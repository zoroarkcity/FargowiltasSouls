...
{
    public class LifeRitual : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[1] >= 0f && projectile.ai[1] < 200f &&
                Main.npc[ai1].active && Main.npc[ai1].type == ModContent.NPCType<LifeChampion>())
            {
                projectile.alpha -= 2;
                if (projectile.alpha < 0)
                    projectile.alpha = 0;

                if (Main.npc[ai1].ai[0] != 2f && Main.npc[ai1].ai[0] != 8f)
                {
